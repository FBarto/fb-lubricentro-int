...
const FORMAS_PAGO = [
  { id: "efectivo", label: "Efectivo", icon: "💵", color: "#16a34a", bg: "#16a34a22" },
  { id: "transferencia", label: "Transferencia", icon: "📱", color: "#3b82f6", bg: "#3b82f622" },
  { id: "debito", label: "Débito", icon: "💳", color: "#8b5cf6", bg: "#8b5cf622" },
  { id: "credito", label: "Crédito", icon: "💰", color: "#f59e0b", bg: "#f59e0b22" },
  { id: "qr", label: "QR / MP", icon: "⬜", color: "#06b6d4", bg: "#06b6d422" },
];
...
function CajaContent() {
  ...
  // Cargar productos reales desde Sheets
  useEffect(() => {
  ...
  const registrarVentaLocal = async () => {
    ...
    try {
      const res = await fetch('/api/caja/venta-local', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          items: carritoLocal.map(i => ({ nombre: i.nombre, cantidad: i.cantidad, precio: i.precio })),
          forma_pago: formaPagoLocal, cliente: clienteLocal, marketing: marketingLocal,
        }),
      });
      if (res.ok) {
    ...
    <div style={{ minHeight: "100vh", background: "#0a0a0a", fontFamily: "'Inter','Segoe UI',sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes cobrado{0%{transform:scale(1)}50%{transform:scale(1.03)}100%{transform:scale(1)}}
        .cobrado-anim{animation:cobrado 0.4s ease-out}
      `}</style>
      ...
      <div style={{ background: "#111", borderBottom: "1px solid #1e1e1e", padding: "10px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[{ id: "pendientes", label: "Pendientes", badge: pendientes.length }, { id: "nueva", label: "Nueva Venta" }, { id: "historial", label: "Historial & Arqueo" }].map((t) => (
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
            ...
            {pendientes.map((o) => {
              ...
              return (
                <div key={o.id} className="row-orden" onClick={() => { setOrdenSeleccionada(o); setFormaPago(""); setClienteCobro(""); setMarketing(""); }}
                  style={{ padding: "12px 16px", borderBottom: "1px solid #141414", background: sel ? "#0f1f40" : "transparent", borderLeft: sel ? "3px solid #3b82f6" : "3px solid transparent", transition: "all 0.15s" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5 }}>
                    ...
                    <span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 20, fontWeight: 800, color: "#3b82f6" }}>{fmt(tot)}</span>
                  </div>
                  <div style={{ fontSize: 13, color: "#ccc", fontWeight: 500, marginBottom: 4, lineHeight: 1.4 }}>
                    {(o.items || []).map((i) => { const n = i.nombre || i.nombre_item || "Servicio"; const c = Number(i.cantidad) || 1; return c > 1 ? `${n} ×${c}` : n; }).join("  ·  ")}
                  </div>
                  {o.patente && (
                    <div style={{ marginBottom: 4 }}>
                      ...
                      {(o.marca || o.modelo) && <span style={{ fontSize: 11, color: "#555", marginLeft: 8 }}>{[o.marca, o.modelo].filter(Boolean).join(' ')}</span>}
                    </div>
                  )}
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 12, color: o.cliente ? "#3b82f6" : "#333" }}>{o.cliente ? `👤 ${o.cliente}` : "Sin nombre"}{o.telefono ? ` · 📞 ${o.telefono}` : ""}</span>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            {!ordenSeleccionada
              ...
              : (
                <div className="fade-in" style={{ flex: 1, display: "flex", flexDirection: "column", padding: 24, overflowY: "auto" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
                    ...
                    <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 38, fontWeight: 800, color: "#fff" }}>{fmt(totalOrden(ordenSeleccionada.items))}</div>
                  </div>

                  <div style={{ background: "#111", borderRadius: 10, overflow: "hidden", marginBottom: 16 }}>
                    {(ordenSeleccionada.items || []).map((item, i) => (
                      <div key={i} style={{ padding: "11px 16px", borderBottom: i < ordenSeleccionada.items.length - 1 ? "1px solid #1a1a1a" : "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <div style={{ fontSize: 14, color: "#ddd", fontWeight: 500 }}>{item.nombre || item.nombre_item || "Servicio"}</div>
                          <div style={{ fontSize: 12, color: "#555" }}>{fmt(item.precio || item.precio_unitario || 0)} × {item.cantidad}</div>
                        </div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>{fmt((Number(item.precio || item.precio_unitario || 0)) * Number(item.cantidad))}</div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {MARKETING.map((m) => (
                        <button key={m} onClick={() => setMarketing(m === marketing ? "" : m)}
                          style={{ padding: "6px 12px", borderRadius: 6, border: `1px solid ${marketing === m ? "#3b82f6" : "#222"}`, background: marketing === m ? "#1d4ed8" : "#111", color: marketing === m ? "#fff" : "#666", fontSize: 12, cursor: "pointer" }}>{m}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                      ...
                      {ordenSeleccionada.es_efectivo === 'true' && (
                        <span style={{ fontSize: 10, background: "#16a34a22", color: "#4ade80", padding: "2px 8px", borderRadius: 4, border: "1px solid #16a34a44" }}>💵 Precio efectivo aplicado</span>
                      )}
                    </div>
                    {ordenSeleccionada.es_efectivo === 'true' ? (
                      <div>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8, opacity: 0.25, pointerEvents: "none", marginBottom: 8 }}>
                          {FORMAS_PAGO.filter(fp => fp.id !== 'efectivo').map((fp) => (
                            <div key={fp.id} style={{ padding: "10px 4px", borderRadius: 8, border: "2px solid #1e1e1e", background: "#111", color: "#333", fontSize: 10, fontWeight: 600, textAlign: "center" }}>
                              <div style={{ fontSize: 18, marginBottom: 3 }}>{fp.icon}</div>{fp.label}
                            </div>
                          ))}
                        </div>
                        <button onClick={() => setFormaPago('efectivo')}
                          style={{ width: "100%", padding: "14px", border: `2px solid ${formaPago === 'efectivo' ? "#16a34a" : "#16a34a66"}`, borderRadius: 9, background: formaPago === 'efectivo' ? "#16a34a22" : "#111", color: formaPago === 'efectivo' ? "#4ade80" : "#16a34a", fontFamily: "'Barlow Condensed',sans-serif", fontSize: 17, fontWeight: 800, cursor: "pointer", letterSpacing: 1 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8 }}>
                        {FORMAS_PAGO.map((fp) => (
                          <button key={fp.id} onClick={() => setFormaPago(fp.id)}
                            style={{ padding: "12px 4px", borderRadius: 8, border: `2px solid ${formaPago === fp.id ? fp.color : "#1e1e1e"}`, background: formaPago === fp.id ? fp.bg : "#111", color: formaPago === fp.id ? fp.color : "#555", fontSize: 11, fontWeight: 600, cursor: "pointer", textAlign: "center", transition: "all 0.15s" }}>
                  ...
                  <button onClick={cobrarOrden} disabled={!formaPago || cobrandoId === ordenSeleccionada.id}
                    ...
                    {cobrandoId === ordenSeleccionada.id ? "Registrando..." : formaPago ? `✓ COBRAR ${fmt(totalOrden(ordenSeleccionada.items))}` : "Seleccioná forma de pago"}
                  </button>
                </div>
              )}
          </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 12, display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 8, alignContent: "start" }}>
              {productosFiltrados.length === 0 && (
                <div style={{ gridColumn: "1/-1", textAlign: "center", color: "#333", fontSize: 13, padding: 40 }}>
                  {productos.length === 0 ? "Cargando productos desde el catálogo..." : "Sin resultados"}
                </div>
              )}
              {productosFiltrados.map((p) => {
                ...
                return (
                  <button key={p.id} className="prod-btn" onClick={() => agregarProducto(p)}
                    ...
                    {p.codigo && <div style={{ fontSize: 10, color: "#444", marginBottom: 4, letterSpacing: 1, fontFamily: "monospace" }}>{p.codigo}</div>}
                    <div style={{ fontSize: 13, fontWeight: 600, color: enCarrito ? "#93c5fd" : "#ccc", marginBottom: 5, lineHeight: 1.3 }}>{p.nombre}</div>
                    {p.categoria && <div style={{ fontSize: 10, color: "#555", marginBottom: 5 }}><span style={{ background: "#1a1a1a", padding: "2px 7px", borderRadius: 4 }}>{p.categoria}</span></div>}
                    {altCodes.length > 0 && (
                      <div style={{ marginBottom: 5, display: "flex", flexWrap: "wrap", gap: 3 }}>
                        {altCodes.slice(0, 4).map((ac, i) => {
                          const [marca, cod] = ac.split(":");
                          return cod ? <span key={i} style={{ fontSize: 9, background: "#1a1a2e", color: "#8b5cf6", padding: "2px 6px", borderRadius: 4, fontFamily: "monospace" }}>{marca}: {cod}</span> : null;
                        })}
                      </div>
                    )}
                    <div style={{ fontSize: 17, fontWeight: 700, color: enCarrito ? "#3b82f6" : "#fff", fontFamily: "'Barlow Condensed',sans-serif" }}>{fmt(p.precio)}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ width: 280, borderLeft: "1px solid #1a1a1a", display: "flex", flexDirection: "column" }}>
                ...
                : carritoLocal.map((item) => (
                  <div key={item.id} style={{ background: "#111", borderRadius: 8, padding: 10, marginBottom: 8, borderLeft: "3px solid #3b82f6" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: 12, color: "#555" }}>{fmt(item.precio)} ×{item.cantidad}</span>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#3b82f6", marginTop: 2, fontFamily: "'Barlow Condensed',sans-serif" }}>{fmt(item.precio * item.cantidad)}</div>
                  </div>
                ))}
            </div>
            <div style={{ padding: "12px 14px", borderTop: "1px solid #1a1a1a" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 10 }}>
                {FORMAS_PAGO.map((fp) => (
                  <button key={fp.id} onClick={() => setFormaPagoLocal(fp.id)}
                    style={{ padding: "8px 4px", borderRadius: 7, border: `1px solid ${formaPagoLocal === fp.id ? fp.color : "#1e1e1e"}`, background: formaPagoLocal === fp.id ? fp.bg : "#111", color: formaPagoLocal === fp.id ? fp.color : "#555", fontSize: 10, fontWeight: 600, cursor: "pointer", textAlign: "center" }}>
                    {fp.icon} {fp.label}
                  </button>
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                ...
                <span style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 24, fontWeight: 800, color: "#fff" }}>{fmt(totalLocal)}</span>
              </div>
      ...
      {tab === "historial" && (
        <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            ...
            {cargandoHistorial ? (
              <div style={{ textAlign: "center", color: "#333", padding: 40, fontSize: 14 }}>Cargando arqueo del día...</div>
            ) : metricas && (
              <div className="fade-in">
                {/* Total del día */}
                <div style={{ background: "#111", borderRadius: 14, padding: "20px 24px", marginBottom: 16, border: "1px solid #1e1e1e", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    <div style={{ fontSize: 11, color: "#555", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>Total facturado hoy</div>
                    <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 48, fontWeight: 800, color: "#fff", lineHeight: 1 }}>{fmt(metricas.total_dia)}</div>
                    <div style={{ fontSize: 13, color: "#555", marginTop: 6 }}>{metricas.cantidad_ventas} venta{metricas.cantidad_ventas !== 1 ? "s" : ""} cobrada{metricas.cantidad_ventas !== 1 ? "s" : ""}</div>
                  </div>
                  <div style={{ display: "flex", gap: 10 }}>
                    <div style={{ textAlign: "center", background: "#1a2744", borderRadius: 10, padding: "12px 16px", border: "1px solid #3b82f622" }}>
                      <div style={{ fontSize: 10, color: "#555", letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Gomería</div>
                      <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 20, fontWeight: 800, color: "#3b82f6" }}>{fmt(metricas.por_origen?.gomeria || 0)}</div>
                    </div>
                    <div style={{ textAlign: "center", background: "#1a1a2e", borderRadius: 10, padding: "12px 16px", border: "1px solid #8b5cf622" }}>
                      <div style={{ fontSize: 10, color: "#555", letterSpacing: 1, textTransform: "uppercase", marginBottom: 4 }}>Caja</div>
                      <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 20, fontWeight: 800, color: "#8b5cf6" }}>{fmt(metricas.por_origen?.caja || 0)}</div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10, marginBottom: 16 }}>
                  {FORMAS_PAGO.map((fp) => {
                    const monto = metricas.por_forma_pago?.[fp.id] || 0;
                    const pct = metricas.total_dia > 0 ? Math.round((monto / metricas.total_dia) * 100) : 0;
                    return (
                      <div key={fp.id} style={{ background: "#111", borderRadius: 10, padding: "14px 12px", border: `1px solid ${monto > 0 ? fp.color + '44' : '#1e1e1e'}`, textAlign: "center" }}>
                        <div style={{ fontSize: 22, marginBottom: 6 }}>{fp.icon}</div>
                        <div style={{ fontSize: 10, color: "#555", marginBottom: 4 }}>{fp.label}</div>
                        <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 18, fontWeight: 800, color: monto > 0 ? fp.color : "#2a2a2a" }}>{fmt(monto)}</div>
                        {monto > 0 && (
                          <div style={{ marginTop: 8 }}>
                            <div style={{ height: 4, background: "#1a1a1a", borderRadius: 2, overflow: "hidden" }}>
                              <div style={{ height: "100%", width: `${pct}%`, background: fp.color, borderRadius: 2, transition: "width .5s" }} />
                            </div>
                            <div style={{ fontSize: 10, color: fp.color, marginTop: 3 }}>{pct}%</div>
                          </div>
                        )}
                      </div>
                    );
                ...
                {/* Origen de clientes */}
                {Object.keys(metricas.por_marketing || {}).length > 0 && (
                  <div style={{ background: "#111", borderRadius: 12, padding: "16px 20px", marginBottom: 20, border: "1px solid #1e1e1e" }}>
                    <div style={{ fontSize: 11, color: "#555", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>Origen de clientes hoy</div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      {Object.entries(metricas.por_marketing).sort((a, b) => b[1] - a[1]).map(([canal, cant]) => (
                        <div key={canal} style={{ background: "#1a1a2e", border: "1px solid #8b5cf633", borderRadius: 8, padding: "6px 14px", display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 13, color: "#ccc" }}>{canal}</span>
                          <span style={{ background: "#8b5cf6", color: "#fff", borderRadius: "50%", width: 22, height: 22, fontSize: 12, fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "center" }}>{cant}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div key={o.id} className="fade-in" style={{ background: "#111", borderRadius: 10, padding: "14px 16px", marginBottom: 10, borderLeft: `3px solid ${o.origen === "gomeria" ? "#3b82f6" : "#8b5cf6"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    {o.patente && <span style={{ fontSize: 11, color: "#93c5fd", background: "#1a2744", padding: "2px 8px", borderRadius: 4, letterSpacing: 1, fontWeight: 700 }}>🚗 {o.patente}</span>}
                    {o.cliente && <span style={{ fontSize: 12, color: "#888" }}>👤 {o.cliente}</span>}
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 20, fontWeight: 800, color: "#16a34a" }}>{fmt(o.total)}</div>
                    <div style={{ fontSize: 10, color: "#444" }}>✓ {o.hora_cobro || o.hora}</div>
                  </div>
                </div>
                <div style={{ fontSize: 12, color: "#555", marginBottom: 6 }}>{(o.items || []).map((i) => `${i.nombre} ×${i.cantidad}`).join(" · ")}</div>
                {(o.forma_pago || o.marketing) && (
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {o.forma_pago && (() => { const fp = FORMAS_PAGO.find(f => f.id === o.forma_pago); return fp ? <span style={{ fontSize: 10, color: fp.color, background: fp.bg, padding: "2px 8px", borderRadius: 4, border: `1px solid ${fp.color}44` }}>{fp.icon} {fp.label}</span> : null; })()}
